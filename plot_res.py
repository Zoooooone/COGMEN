...
def plot_gnn_heads(data, args):
    ...
    for i in range(iter_num):
        ...
        train_losses, test_f1s = np.array(train_losses), np.array(test_f1s)
        best_test_f1 = np.max(test_f1s)
        best_test_f1_epoch = np.argmax(test_f1s) + 1
        ...
        # plt.fill_between(epochs_range, test_f1s - np.std(test_f1s), test_f1s + np.std(test_f1s), alpha=0.2)
...
def plot_windows(data, zoom_in=True):
    ...
    if zoom_in:
        ...
        plt.savefig("results/img/windows/plot_res_windows_zoom_in.png")
    else:
        ...
        for i in range(iter_num):
            ...
            plt.subplot(grid[0, 0])
            plt.title("loss curve", fontsize=font_size // 2)
            plt.plot(epochs_range, train_losses, "o-", label=f"window = {window}", linewidth=1, color=line_color, markersize=marker_size // 2)
            plt.xlabel("epochs", fontsize=font_size // 2)
            plt.ylabel("losses", fontsize=font_size // 2)
            
            plt.subplot(grid[0, 1])
            plt.title("learning curve", fontsize=font_size // 2)
            plt.plot(epochs_range, test_f1s, "o-", label=f"window = {window}", linewidth=1, color=line_color, markersize=marker_size // 2)
            plt.xlabel("epochs", fontsize=font_size // 2)
            plt.ylabel("F1", fontsize=font_size // 2)
        set_ticks_size(font_size=font_size // 2)
        plt.legend(loc=2, bbox_to_anchor=(0.6, 0.7), prop={"size": 7})
...
def plot_all_label_metrics(args):
    labels = {
        "hap": "happy",
        "sad": "sad",
        "neu": "neutral",
    ...
    modalities = ["a", "t", "v", "at", "av", "tv", "atv"]
    plt.figure(dpi=200, figsize=(12, 9))
    
    for i in range(len(modalities)):
        with open(f"results/label_metrics_pkl/label_metrics_{args.dataset}_{modalities[i]}.pkl", "rb") as file:
    ...
    plt.xlabel("epochs")
    plt.ylabel("F1")
    plt.legend(loc=2, bbox_to_anchor=(1, 1), prop={"size": 14})
    plt.suptitle(f"dataset: {args.dataset.upper()}", fontsize=12)
    plt.savefig(f"results/img/label_metrics/all_{args.dataset}.png")
...
def main(args):
    ...
    if args.plot == "windows":
        ...
        plot_windows(res_windows, zoom_in=False)
    ...
    if args.plot == "all_modalities_label_metrics":
        plot_all_label_metrics(args)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="plot_res.py")
    parser.add_argument(
        "--plot", type=str, help="Types of figures", choices=["gnn_heads", "windows", "label_metrics", "all_modalities_label_metrics"], default="gnn_heads"
    )
    parser.add_argument(
